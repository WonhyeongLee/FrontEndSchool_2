...
const f1 = fetch('https://raw.githubusercontent.com/paullabkorea/coronaVaccinationStatus/main/data/data.json')
...
const snack2 = () => Promise.resolve('와, 맛있겠다!') // 4번

async function mySnack() {
	console.log('치즈 케이크 주세요!') // 3번
    const res = await snack2() // 4번
    console.log(res) // 6번
}
...
const myCoffee2 = async () => {
    await cook(2000);
    return '커피';
};
const myCookie2 = async () => {
    await cook(5000);
    return '쿠키';
};
